...
@interface privateAddVC ()<UITextViewDelegate,UINavigationControllerDelegate,UIImagePickerControllerDelegate>
{
    int c;
...
- (void)viewDidLoad {
    ...
    UIColor *lightPink = [UIColor colorWithRed:255/255.0 green:231/255.0 blue:251/255.0 alpha:1];
    self.navigationController.view.backgroundColor = lightPink;
    ...
    // 在 self.privateNote 中滑动，而不是 self.view
...
-(void)textViewDidChange:(UITextView *)textView {
    self.privateNote.font = [UIFont fontWithName:@"Arial" size:18.0];
...
-(void)CompleteClick {
    ...
    if(self.privateNote.text.length > 0){
        // 写入 plist
...
-(void)fileHandleOperate {
    ...
    self.textStr = self.privateNote.text;
...
-(void)insertImage:(UIImage *)image {
    self.exit = YES;
    NSString *str = [self.privateNote.text stringByAppendingString:@"\n"];
    ...
    self.privateNote.attributedText = string;
